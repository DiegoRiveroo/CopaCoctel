...
async function renderDrinks(drinks = []) {
  ...
  const selectedDrinkId = drinkSelectEl.value;
  ...
  const selectedStillAvailable = options.some(
      option => String(option.id) === String(selectedDrinkId)
  );

  if (selectedStillAvailable) {
    drinkSelectEl.value = selectedDrinkId;
  }
  ...
  drinksGridEl.innerHTML = drinks.map(drink => {
    ...
    const selectedClass = selectedStillAvailable && String(drink.id) === String(selectedDrinkId)
        ? "selected-card"
        : "";

    return `
      <article class="drink-card ${disabledClass} ${selectedClass}" data-drink-id="${escapeHtml(drink.id)}">
        <div class="edition-badge">${getEditionLabel(drink.round)}</div>
        <div class="drink-icon">${getDrinkIcon(drink.round)}</div>
        <h3>${escapeHtml(drink.name)}</h3>
        <p class="drink-meta">${escapeHtml(drink.teamName)} ${drink.table ? `· Mesa ${escapeHtml(drink.table)}` : ""}</p>
...
setInterval(async () => {
  const hayTragoSeleccionado = drinkSelectEl?.value;
  const hayEstrellasMarcadas = [...document.querySelectorAll(".rating-group input")]
      .some(input => input.value);

  if (hayTragoSeleccionado || hayEstrellasMarcadas) {
    return;
  }
...
setInterval(() => {
  const hayTragoSeleccionado = drinkSelectEl && drinkSelectEl.value;
  const hayEstrellasMarcadas = Array.from(document.querySelectorAll(".rating-group input"))
      .some(input => input.value);
